...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	float cx = 50.0f;
	float cy = 50.0f;
	// Primul patrat o sa se roteasca in jurul centrului sau
	...
	// Al doilea patrat o sa pulseze din centrul sau
	modelMatrix = glm::mat3(1);
	...
	// Al treilea patrat o sa se miste sus si jos pe axa OY
	modelMatrix = glm::mat3(1);
	...
	// Al patrulea patrat e bonus
	// Numai in jurul patratului 3 nu vrea sa se invarta
	// modelMatrix = glm::mat3(1);
	...
	modelMatrix *= Transform2D::Scale(sin(scaleX), sin(scaleY));
	modelMatrix *= Transform2D::Translate(650, 250);
	angularStep += deltaTimeSeconds * 0.05f;
	scaleX += deltaTimeSeconds * 100;
	scaleY += deltaTimeSeconds * 100;
