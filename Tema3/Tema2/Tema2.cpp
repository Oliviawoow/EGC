...
void Tema2::Init()
{
	...
	const string textureLoc = "Source/Laboratoare/Tema2/Textures/";
	{
		...
		texture->Load2D((textureLoc + "rock-sand.png").c_str(), GL_REPEAT);
		mapMyTextures["rock-sand"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "red-marble.png").c_str(), GL_REPEAT);
		mapMyTextures["red-marble"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "blue-marble.png").c_str(), GL_REPEAT);
		mapMyTextures["blue-marble"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "yellow-marble.png").c_str(), GL_REPEAT);
		mapMyTextures["yellow-marble"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "orange-marble.png").c_str(), GL_REPEAT);
		mapMyTextures["orange-marble"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "green-marble.png").c_str(), GL_REPEAT);
		mapMyTextures["green-marble"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "purple-marble.png").c_str(), GL_REPEAT);
		mapMyTextures["purple-marble"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "turcoaz-marble.png").c_str(), GL_REPEAT);
		mapMyTextures["turcoaz-marble"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "grey-marble.png").c_str(), GL_REPEAT);
		mapMyTextures["grey-marble"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "moon.png").c_str(), GL_REPEAT);
		mapMyTextures["moon"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "stone.png").c_str(), GL_REPEAT);
		mapMyTextures["stone"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "sun.png").c_str(), GL_REPEAT);
		mapMyTextures["sun"] = texture;
	}
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "diamond.png").c_str(), GL_REPEAT);
		mapMyTextures["diamond"] = texture;
	}
	...
	std::vector<glm::vec3> vertices1 =
	{
		glm::vec3(0, 0, 0),
		glm::vec3(0, 0, -lenght),
		glm::vec3(4 * lenght, 0, -lenght),
		glm::vec3(4 * lenght, 0, 0),

		glm::vec3(0, 0, 0),
		glm::vec3(2 * lenght, 5 * lenght,  -lenght / 2),
		glm::vec3(4 * lenght, 0, 0),
	...
	meshes["cub"] = new Mesh("generated cub1");
	meshes["cub"]->InitFromData(vertices, indices);

	Mesh* mesh3 = new Mesh("piramida");
	mesh3->InitFromData(vertices1, normals1, textureCoords1, indices1);
	...
	{
		Shader* shader2 = new Shader("ShaderTema2Texturi");
		shader2->AddShader("Source/Laboratoare/Tema2/Shaders/Tema2_VertexShaderTexturi.glsl", GL_VERTEX_SHADER);
		shader2->AddShader("Source/Laboratoare/Tema2/Shaders/Tema2_FragmentShaderTexturi.glsl", GL_FRAGMENT_SHADER);
...
void Tema2::RenderSimpleMeshTexture(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, Texture2D* texture1)
{
...
void Tema2::Update(float deltaTimeSeconds)
{
	...
	for (int i = 0; i < platforme.size(); i++) {
		...
		if (platforme[i].getCuloare() == 2) {
			...
			RenderSimpleMeshTexture(meshes["box"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["red-marble"]);
		}
		// albastru = 1
		if (platforme[i].getCuloare() == 1) {
			// culoare = glm::vec3(0.1, 0, 1);
			RenderSimpleMeshTexture(meshes["box"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["blue-marble"]);
		}
		// galben = 3
		if (platforme[i].getCuloare() == 3) {
			// culoare = glm::vec3(1, 1, 0.2);
			RenderSimpleMeshTexture(meshes["box"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["yellow-marble"]);
		}
		// portocaliu = 4
		if (platforme[i].getCuloare() == 4) {
			// culoare = glm::vec3(0.9, 0.5, 0);
			RenderSimpleMeshTexture(meshes["box"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["orange-marble"]);
		}
		// verde = 5
		if (platforme[i].getCuloare() == 5) {
			// culoare = glm::vec3(0, 0.9, 0.5);
			RenderSimpleMeshTexture(meshes["box"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["green-marble"]);
		}
		// mov = 6
		if (platforme[i].getCuloare() == 6) {
			// culoare = glm::vec3(0.7, 0.3, 0.7);
			RenderSimpleMeshTexture(meshes["box"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["purple-marble"]);
		}
		// turcoaz = 7
		if (platforme[i].getCuloare() == 7) {
			// culoare = glm::vec3(0.7, 1, 0.9);
			RenderSimpleMeshTexture(meshes["box"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["turcoaz-marble"]);
		}
		// gri = 8
		if (platforme[i].getCuloare() == 8) {
			// culoare = glm::vec3(0.8, 0.8, 0.8);
			RenderSimpleMeshTexture(meshes["box"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["grey-marble"]);
		}
	...
	for (int i = 0; i < cuburi.size(); i++) {
		modelMatrix = glm::mat4(1);
		modelMatrix *= Tema2_Transform3D::Translate(cuburi[i].getX(), cuburi[i].getY(), platforme[i].getZ());
		modelMatrix *= Tema2_Transform3D::Scale(cuburi[i].getScale_x(), cuburi[i].getScale_y(), cuburi[i].getScale_z());
		RenderSimpleMeshTexture(meshes["box"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["stone"]);
	}

	// piramida
	if (piramide.size() < 20) {
		addPiramide(piramide, z);
	}
	for (int i = 0; i < piramide.size(); i++) {
		modelMatrix = glm::mat4(1);
		modelMatrix *= Tema2_Transform3D::Translate(piramide[i].getX(), piramide[i].getY(), piramide[i].getZ());
		modelMatrix *= Tema2_Transform3D::Scale(piramide[i].getScale_x(), piramide[i].getScale_y(), piramide[i].getScale_z());
		RenderSimpleMeshTexture(meshes["piramida"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["rock-sand"]);
	}

	// scot piramidele care nu se mai vad si le pun in fata
	for (int i = 0; i < piramide.size(); i++) {
		if (piramide[i].getZ() > camera->position.z) {
			piramide[i].setZ(z - 2.f);
				z -= 2;
		}
	}

	// colectabile
	if (colectabile.size() < 20) {
		addColectabile(colectabile);
	}
	for (int i = 0; i < colectabile.size(); i++) {
		modelMatrix = glm::mat4(1);
		modelMatrix *= Tema2_Transform3D::Translate(colectabile[i].getX(), colectabile[i].getY(), platforme[i].getZ() + 0.1);
		modelMatrix *= Tema2_Transform3D::Scale(colectabile[i].getScale_x(), colectabile[i].getScale_y(), colectabile[i].getScale_z());
		RenderSimpleMeshTexture(meshes["piramida"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["diamond"]);
		modelMatrix = glm::mat4(1);
		modelMatrix *= Tema2_Transform3D::Translate(colectabile[i].getX(), colectabile[i].getY(), platforme[i].getZ());
		modelMatrix *= Tema2_Transform3D::Scale(colectabile[i].getScale_x(), colectabile[i].getScale_y(), colectabile[i].getScale_z());
		modelMatrix *= Tema2_Transform3D::RotateOX(110);
		RenderSimpleMeshTexture(meshes["piramida"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["diamond"]);
	}
	...
	if (power == false) {
		modelMatrix = glm::mat4(1);
		modelMatrix *= Tema2_Transform3D::Translate(1.5, 3.5, -2);
		modelMatrix *= Tema2_Transform3D::Translate(0, 0, translateZ_sphere);
		RenderSimpleMeshTexture(meshes["sphere"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["moon"]);
	} else if (power == true) {
		modelMatrix = glm::mat4(1);
		modelMatrix *= Tema2_Transform3D::Translate(1.5, 3.5, -2);
		modelMatrix *= Tema2_Transform3D::Translate(0, 0, translateZ_sphere);
		RenderSimpleMeshTexture(meshes["sphere"], shaders["ShaderTema2Texturi"], modelMatrix, mapMyTextures["sun"]);
	}
	...
	// coliziuni cu obstacolul
	for (int i = 0; i < cuburi.size(); i++) {
		coliziuneX = max(cuburi[i].getX(), min(translateX_sphere, cuburi[i].getX() + cuburi[i].getScale_x()));
		coliziuneY = max(cuburi[i].getY(), min(translateY_sphere, cuburi[i].getY() + cuburi[i].getScale_y()));
		coliziuneZ = max(platforme[i].getZ(), min(translateZ_sphere, platforme[i].getZ() + cuburi[i].getScale_z()));
		...
		if (distance < scaleX_sphere / 2  && cuburi[i].getScale_x() != 0 && cuburi[i].getScale_y() != 0 && cuburi[i].getScale_z() != 0) {
			cout << "Coliziune obstacol" << '\n';
			cout << "GAME OVER" << '\n';
			exit(1);
		}
	}

	// coliziune colectabil
	for (int i = 0; i < colectabile.size(); i++) {
		coliziuneX = max(colectabile[i].getX(), min(translateX_sphere, colectabile[i].getX() + colectabile[i].getScale_x()));
		coliziuneY = max(colectabile[i].getY(), min(translateY_sphere, colectabile[i].getY() + colectabile[i].getScale_y()));
		coliziuneZ = max(platforme[i].getZ(), min(translateZ_sphere, platforme[i].getZ() + colectabile[i].getScale_z()));

		distance = sqrt(((coliziuneX - translateX_sphere) * (coliziuneX - translateX_sphere)) +
			((coliziuneY - translateY_sphere) * (coliziuneY - translateY_sphere)) +
			((coliziuneZ - translateZ_sphere) * (coliziuneZ - translateZ_sphere)));

		if (distance < scaleX_sphere / 2 && colectabile[i].getScale_x() != 0 && colectabile[i].getScale_y() != 0 && colectabile[i].getScale_z() != 0) {
			cout << "Coliziune colectabil" << '\n';
			scor += 10;
		}
	}
	...
	// viteza
	modelMatrix = glm::mat4(1);
	modelMatrix *= Tema2_Transform3D::Translate(translateX_bara + 3.05, translateY_bara, translateZ_bara);
	modelMatrix *= Tema2_Transform3D::Scale(scaleX_bara, scaleY_bara, scaleZ_bara);
	RenderSimpleMesh(meshes["box"], shaders["ShaderTema2Placute"], modelMatrix, glm::vec3(1, 1, 1));

	modelMatrix = glm::mat4(1);
	modelMatrix *= Tema2_Transform3D::Translate(translateX_combustibil + 3, translateY_combustibil, translateZ_combustibil);
	...
	// viata
	modelMatrix = glm::mat4(1);
	modelMatrix *= Tema2_Transform3D::Translate(translateX_combustibil + 0.25, translateY_combustibil - 1, translateZ_combustibil);
	...
	modelMatrix = glm::mat4(1);
	modelMatrix *= Tema2_Transform3D::Translate(translateX_combustibil + 1, translateY_combustibil - 1, translateZ_combustibil);
	modelMatrix *= Tema2_Transform3D::Scale(scaleX_viata2, scaleY_viata2, scaleZ_viata2);
	RenderSimpleMesh(meshes["sphere"], shaders["ShaderTema2Placute"], modelMatrix, culoareViata);

	modelMatrix = glm::mat4(1);
	modelMatrix *= Tema2_Transform3D::Translate(translateX_combustibil + 1.75, translateY_combustibil - 1, translateZ_combustibil);
	modelMatrix *= Tema2_Transform3D::Scale(scaleX_viata3, scaleY_viata3, scaleZ_viata3);
	RenderSimpleMesh(meshes["sphere"], shaders["ShaderTema2Placute"], modelMatrix, culoareViata);

	// scor
	modelMatrix = glm::mat4(1);
	modelMatrix *= Tema2_Transform3D::Translate(translateX_bara + 6.1, translateY_bara, translateZ_bara);
	modelMatrix *= Tema2_Transform3D::Scale(scaleX_bara, scaleY_bara, scaleZ_bara);
	RenderSimpleMesh(meshes["box"], shaders["ShaderTema2Placute"], modelMatrix, glm::vec3(1, 1, 1));

	modelMatrix = glm::mat4(1);
	modelMatrix *= Tema2_Transform3D::Translate(translateX_combustibil + 5.97, translateY_combustibil, translateZ_combustibil);
	modelMatrix *= Tema2_Transform3D::Translate(+scor / 2000, -scaleY_viteza / 2, -scaleZ_viteza / 2);
	modelMatrix *= Tema2_Transform3D::Scale(scor / 1000, scaleY_viteza, scaleZ_viteza);
	RenderSimpleMesh(meshes["box"], shaders["ShaderTema2Placute"], modelMatrix, glm::vec3(1, 0.7, 0));
}
...
// functie care imi genereaza random daca apare sau nu un obstacol pe platforma
int Tema2::myRandomAppear() {
	int x;
	x = rand() % 100;
	if (x <= 80) {
		x = 0;
...
// functie care imi adauga obstacole
void Tema2::addObstacole(std::vector<Cub>& cuburi) {
	float x, y, z;
	...
	x = -1.9;
	Cub cubNew1;
	cubNew1.setAll(x, y, z, scaleX, scaleY, scaleZ);
	cuburi.push_back(cubNew1);
	...
	x = 0;
	Cub cubNew2;
	cubNew2.setAll(x, y, z, scaleX, scaleY, scaleZ);
	cuburi.push_back(cubNew2);
	...
	x = 1.9;
	Cub cubNew3;
	cubNew3.setAll(x, y, z, scaleX, scaleY, scaleZ);
	cuburi.push_back(cubNew3);
}
...
// functie care imi adauga piramide
void Tema2::addPiramide(std::vector<Piramida>& piramide, int& z) {
	float x, y, zz;
	float scaleX, scaleY, scaleZ;
	y = -0.45;
	zz = z - 2;
	...
	x = -5.5;
	Piramida piramidaNew1;
	piramidaNew1.setAll(x, y, zz, scaleX, scaleY, scaleZ);
	piramide.push_back(piramidaNew1);

	x = 2.5;
	Piramida piramidaNew2;
	piramidaNew2.setAll(x, y, zz, scaleX, scaleY, scaleZ);
	piramide.push_back(piramidaNew2);

	z = zz;
}

// functie care imi adauga colectabile
void Tema2::addColectabile(std::vector<Piramida>& colectabile) {
	float x, y, z;
	float scaleX, scaleY, scaleZ;
	y = 0.1;
	z = 0;
	...
	x = -2.2;
	Piramida colectabil1;
	colectabil1.setAll(x, y, z, scaleX, scaleY, scaleZ);
	colectabile.push_back(colectabil1);
	...
	x = -0.2;
	Piramida colectabil2;
	colectabil2.setAll(x, y, z, scaleX, scaleY, scaleZ);
	colectabile.push_back(colectabil2);
	...
	x = 1.7;
	Piramida colectabil3;
	colectabil3.setAll(x, y, z, scaleX, scaleY, scaleZ);
	colectabile.push_back(colectabil3);
}

// functie care imi genereaza random daca apare sau nu un colectabil pe platforma
int Tema2::myRandomAppearPoz() {
	int x;
	x = rand() % 100;
	if (x <= 70) {
		x = 0;
