...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	// BONUS CUB CARE TOPAIE:
	mijloc = (stanga + dreapta) * 0.5f;
	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(mijloc, 0.5f, 0.5f);
	modelMatrix *= Transform3D::RotateOZ(unghisalt);
	modelMatrix *= Transform3D::Translate(stanga - mijloc, 0, 0);
	modelMatrix *= Transform3D::Scale(0.5f, 0.5f, 0.5f);
	RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
	unghisalt -= 2 * deltaTimeSeconds;
	if (unghisalt <= -3) {
		stanga = dreapta;
		dreapta += 1;
		unghisalt = 0.f;
