...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(0, 2.3, 0));
		modelMatrix = glm::rotate(modelMatrix, RADIANS(50.0f), glm::vec3(1, 0, 0));
...
void Laborator5::OnKeyPress(int key, int mods)
{
	...
	{
		var = false;
		projectionMatrix = glm::ortho(left, right, bottom, top, 2.0f, 50.0f);
	}
	if (key == GLFW_KEY_P)
	{
		var = true;
		projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, 2.0f, 50.0f);
	}
}
